  ...
  maven 'maven'
    ...
    stages{
        ...
        stage ('Deploy'){
            steps{
                    sshagent(['k8s']) {
                        sh "sudo scp -o StrictHostKeyChecking=no service.yaml deployment.yaml ubuntu@100.24.53.137:/home/ubuntu/"
                        sh "sudo ssh ubuntu@100.24.53.137 kubectl apply -f . --validate=false"
                        //script{
                            //try{
